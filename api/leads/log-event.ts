import { createLeadEvent, validateLeadEventPayload } from "../../lib/leads/lead-events";
...
export default async function handler(req: ApiRequest, res: ApiResponse): Promise<void> {
  ...
  if (req.method !== "POST") {
    return sendJson(res, 405, {
      ...
      error: {
        ...
        message: "Use POST to log a lead event."
      },
      request_id: rid
    });
  }
  ...
  if (!validation.ok || !validation.event) {
    return sendJson(res, 400, {
      ...
      error: {
        ...
        message: "Lead event failed validation.",
  ...
  // Persist event as immutable activity record after storage connector is live.
  // Do not log raw PII or sensitive borrower data.

  return sendJson(res, 202, {
    ok: true,
    message: "Lead event accepted for review-safe activity logging.",
    event,
    stored: false,
