/**
 * Manual Hubspot sync retry endpoint
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Production wiring: load sync job from database, verify target_system === "hubspot", then enqueue retry.
  ...
  return res.status(202).json({
    ok: true,
    message: "Hubspot sync retry queued for review-safe execution.",
    sync_job: queued
  });
}
