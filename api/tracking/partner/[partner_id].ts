import { slugifyTrackingPart } from "../../../lib/tracking/tracking-id";
...
export default async function handler(req: Req, res: Res) {
  ...
  const partnerId = getQueryValue(req, "partner_id").trim();
  ...
  const partnerSlug = slugifyTrackingPart(partnerId, "partner");
  ...
  return send(res, 200, {
    ...
    records: [
      {
        tracking_id: `trk_${partnerSlug}_demo-readiness-referral_001`,
        campaign_id: "camp_demo-readiness_general_001",
        generated_url: `${process.env.TRACKING_BASE_URL || "https://YOUR_VERCEL_DOMAIN.vercel.app"}/r/trk_${partnerSlug}_demo-readiness-referral_001`,
    ...
    source: "stub",
    note: "Partner tracking records are stubbed in Phase 28. Wire to storage before using for production attribution or partner reporting."
  });
}
