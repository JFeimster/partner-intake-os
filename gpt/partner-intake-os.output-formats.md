...
This file defines reusable output formats for Partner Intake OS. Use the format that matches the user’s request. If the user asks for JSON only, return valid JSON only with no Markdown fence or commentary.
...
## 4. JSON Partner Record

Use this when the user asks for a structured partner record.
...
## 8. Welcome Email Draft

Use only when asked to draft partner-facing copy.

```md
Subject: Welcome to the Partner Command Center
